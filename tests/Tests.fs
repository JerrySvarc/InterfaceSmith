...
let RenderingCodeReplacementTests = testList "Replace Function Tests" [
    testCase "Replace a root element" <| fun _ ->
    ...
    testCase "Replace all items in HtmlList" <| fun _ ->
        ...
        | HtmlList(listType, items, handlers) ->
            ...
            items |> List.iter (fun item ->
                Expect.equal item replacement "Each item should be replaced"
            )
            Expect.isEmpty handlers "Handlers should remain empty"
        | _ -> failtest "Result should be an HtmlList"

    testCase "Replace item in nested HtmlObject" <| fun _ ->
        let original = HtmlObject("div", ["header"; "content"; "footer"],
            Map.ofList [
                "header", HtmlElement(H1, [], Empty, [])
                "content", HtmlObject("section", ["title"; "body"],
                    Map.ofList [
    ...
    testCase "Replace wrapped code in CustomWrapper" <| fun _ ->
        let original = CustomWrapper {
            Tag = Div
            Attributes = []
        ...
        | CustomWrapper wrapper ->
            Expect.equal wrapper.WrappedCode replacement "Should replace the wrapped code"
        | _ -> failtest "Result should be a CustomWrapper"

    testCase "Replace child in CustomWrapper" <| fun _ ->
        let original = CustomWrapper {
            Tag = Div
            Attributes = []
            WrappedCode = HtmlElement(P, [], Empty, [])
    ...
    testCase "Replace in HtmlObject with non-existent key" <| fun _ ->
        let original = HtmlObject("div", ["notHeader"], Map.ofList [("header", HtmlElement(H1, [], Empty, []))], [])
        let replacement = HtmlElement(H2, [], Empty, [])
        let result = replace [0] replacement original
        Expect.equal result original "Should not modify the object for non-existent key"

    testCase "Replace CustomElement" <| fun _ ->
        let original = CustomElement { Tag = Span; Attributes = []; CustomInnerValue = "Hello"; EventHandlers = [] }
        let replacement = HtmlElement(P, [], Empty, [])
        let result = replace [] replacement original
        Expect.equal result replacement "Should replace CustomElement at root level"

    testCase "Replace Hole" <| fun _ ->
        let original = Hole(Named "placeholder")
        let replacement = HtmlElement(Div, [], Empty, [])
        let result = replace [] replacement original
        Expect.equal result replacement "Should replace Hole at root level"

    testCase "Deep nested replacement" <| fun _ ->
        let original = HtmlObject("div", ["content"],
            Map.ofList [
                "content", HtmlList(OrderedList, [
                    HtmlElement(Li, [], Empty, []);
                    CustomWrapper {
                        Tag = Span
                        Attributes = []
    ...
    testCase "Replace with complex event handlers" <| fun _ ->
        let original = HtmlElement(Tag.Input, [], Empty, [("click", JSFunction("handleClick", "console.log('clicked')"))])
        let replacement = HtmlElement(A, [], Empty, [("hover", JSFunction("handleHover", "console.log('hovered')"))])
        let result = replace [] replacement original
        Expect.equal result replacement "Should replace element including event handlers"

    testCase "Replace in CustomWrapper with multiple children" <| fun _ ->
        let original = CustomWrapper {
            Tag = Div
            Attributes = []
            WrappedCode = HtmlElement(P, [], Empty, [])
        ...
        match result with
        | CustomWrapper wrapper ->
            Expect.equal (List.item 1 wrapper.Children) replacement "Should replace the second child in CustomWrapper"
        | _ -> failtest "Expected CustomWrapper"

    testCase "Attempt to replace in Hole" <| fun _ ->
