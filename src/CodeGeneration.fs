...
let rec combineDataAndCode (inputData: Json) (inputCode: RenderingCode) : RenderingCode =
    ...
    | JObject obj, Sequence (objects) ->
        let newCodes =
            List.map2 (fun (key, value) object -> (combineDataAndCode value object)) (obj |> Map.toList) objects
...
let rec generateCode (code: RenderingCode) =
    ...
    | HtmlList (listType, numbered, data, code) ->
        ...
        | JArray array ->
            let topTag =
                match listType with
                | List -> if numbered then "ol" else "ul"
                | Table -> "table"
            ...
            let html: string =
                ...
                |> List.map (fun item ->
                    ...
                    | JObject obj, Sequence (objects) ->
                        let newCodes =
                            List.map2
                                (fun (key, value) object -> (combineDataAndCode value object))
                                (obj |> Map.toList)
                                objects
                    ...
                    | JArray array, HtmlList (listType, numbered, data, code) ->
                        ...
                        + ">"
                    | JNull, _ -> ""
                    | _, _ -> generateCode (combineDataAndCode item code))
                |> String.concat "\n"
    ...
    | Sequence (items) ->
        generateCode (
            HtmlElement(
                ...
                List.Empty,
                Constant(
                    List.map (fun item -> generateCode item) (items)
                    |> String.concat "\n"
                )
            )
        )