...
let Sidebar (model: Model) (dispatch: Msg -> unit) =
    Html.div [
        prop.className (sprintf "relative bg-gray-800 text-white transition-all duration-300 %s"
                                (if model.IsSidebarOpen then "w-64" else "w-16"))
        prop.children [
            // Toggle button
            Html.button [
                prop.className (sprintf "absolute top-4 %s p-2 rounded hover:bg-gray-700 transition-colors duration-200"
                                        (if model.IsSidebarOpen then "right-4" else "left-1/2 -translate-x-1/2"))
                prop.onClick (fun _ -> dispatch ToggleSidebar)
                prop.children [
                    if model.IsSidebarOpen then
                        ReactBindings.React.createElement(arrowLeftLine, createObj [
                            "size" ==> 20
                            "color" ==> "#FFFFFF"
                        ],[])
                    else
                        ReactBindings.React.createElement(arrowRightLine, createObj [
                            "size" ==> 20
                            "color" ==> "#FFFFFF"
                        ],[])
                ]
                            ...
                            prop.children (
                                ...
                                |> List.mapi (fun index (pageId, page) ->  // Use mapi to get the index
                                    Html.div [
                                        ...
                                        prop.className (sprintf "flex items-center p-2 hover:bg-gray-700 cursor-pointer rounded %s"
                                                                (if model.ActiveTabId = Some pageId then "bg-gray-700" else ""))
    ...
    let onDrop (e: DragEvent) (targetTabId: Guid) =
        ...
        | Some draggedTabId when draggedTabId <> targetTabId ->
            let newOrder =
                ...
                |> List.fold (fun acc tab ->
                    if tab.Id = targetTabId
                    then acc @ [model.OpenTabs |> List.find (fun t -> t.Id = draggedTabId)] @ [tab]
                    else acc @ [tab]
                ) []
                    ...
                    for tab in model.OpenTabs do
                        ...
                        | Some page ->
                            Html.div [
                                ...
                                prop.className (sprintf "px-4 py-2 flex items-center cursor-move %s"
                                                        (if model.ActiveTabId = Some tab.Id then "bg-gray-200" else ""))
                                prop.draggable true
                                ...
                                prop.children [
                                    ...
                                    Html.button [
                                        ...
                                        prop.children [
                                            ReactBindings.React.createElement(xIcon, createObj [
                                                "size" ==> 16
                                                "color" ==> "#000000"
                                            ],[])
                                        ]
